...
export class TimetableService {
  ...
  async autoGenerate(dto: AutoGenerateDto) {
    ...
    const duration = dto.periodDurationMinutes;
    const breakAfter = dto.breakAfterPeriod ?? 2;
    const breakDur = dto.breakDurationMinutes ?? 20;

    // Build period-queue: expand each subject to N slots based on periodsPerWeek
    type PeriodEntry = { subjectId: string; subjectName: string; teacherId: string; teacherName: string };
    const queue: PeriodEntry[] = [];
    for (const cs of dto.classSubjects) {
      for (let i = 0; i < cs.periodsPerWeek; i++) {
        queue.push({ subjectId: cs.subjectId, subjectName: cs.subjectName, teacherId: cs.teacherId, teacherName: cs.teacherName });
      }
    }
    ...
    for (const day of days) {
      let [startH, startM] = dto.schoolDayStartTime.split(':').map(Number);
      let periodCount = 0;

      while (qIdx < queue.length) {
        // Add break after N periods
        ...
        if (startH >= 17) break; // Don't schedule past 5PM
