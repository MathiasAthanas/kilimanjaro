...
export function getServiceUrls(configService: ConfigService): ServiceUrls {
  return {
    ...
    academic: configService.get<string>('ACADEMIC_SERVICE_URL') || 'http://localhost:3013',
    finance: configService.get<string>('FINANCE_SERVICE_URL') || 'http://localhost:3014',
    notification: configService.get<string>('NOTIFICATION_SERVICE_URL') || 'http://localhost:3005',
    analytics: configService.get<string>('ANALYTICS_SERVICE_URL') || 'http://localhost:3416',
  };
}
